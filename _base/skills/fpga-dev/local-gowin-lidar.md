...
## Робастную σ нельзя сравнивать с паспортным RMS (2026-07-30)

`msop_sigma.py` печатает две оценки разброса: обычную σ и робастную по MAD
(`1.4826 * MAD`, коммит `af310740`). Робастная — НАШЕ диагностическое
дополнение, в даташите LTDC-X3 её нет: чип нормируется только по
**single-shot RMS** (30 пс при `HIGHRES=0`, 25 при 2×, 20 при 4×).

Грабля: сравнивать робастную σ с паспортной нормой. RMS считается по всем
выстрелам и включает хвосты, а робастная оценка хвосты по построению
отбрасывает, поэтому на тяжёлохвостом распределении она заведомо меньше
обычной. «Соответствие паспорту», полученное так, есть артефакт выбора
оценщика, а не свойство железа. В отчёте по СКО это дважды дало ложный вывод
(«ядро близко к паспортным 30 пс», «19.8 пс лучше паспортных 30 пс») —
исправлено 30.07.2026.

Правило: против даташита ставить ТОЛЬКО обычную σ. Реальная картина стенда
по этой метрике — 64.9 пс на базе против нормы 30 (2.2× хуже) и 41.6 пс на
лучшей конфигурации против нормы 20 при `HIGHRES=2` (2.1× хуже); норму тракт
не держит ни в одной конфигурации.

Робастная σ полезна для другого — для структуры распределения. Диагностика
в ОТНОШЕНИИ обычной σ к робастной: ~1.0 значит гауссов шум, а 2.5 или 6.5 —
тяжёлые хвосты либо гребёнка, то есть дискретная систематика вместо белого
шума. Именно падение этого отношения с 6.5 до 1.0 показало, что `HIGHRES=2`
рассыпал гребёнку, и для картинки это было важнее самих пикосекунд.
