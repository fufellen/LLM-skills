...
## Сигнал энкодера R120M: геометрия диска и два постоянных дефекта (20.08.2026)

Источник: четыре независимых VCD-дампа фронтов из lconf (кадр `proto_type 0x10`,
`$timescale 20ns` = такт 50 МГц), сняты 20.08.2026 10:41…10:50 —
`encoder.vcd`, `encoder2.vcd`, `encoder8.vcd`, `encoderlast.vcd`, по 4096 фронтов
≈ 1.58 с ≈ 38…39 оборотов каждый. Все числа ниже совпали во всех четырёх записях.
Графики: `C:\Users\User\Downloads\encoder_zero_mark_oborot.png`,
`encoder_zero_mark_zoom.png`, `encoder_defect_ribs.png`.

**Штатная геометрия.**

- Оборот 39.974 / 39.975 / 40.016 / 40.076 мс = 25.02 / 25.02 / 24.99 / 24.95 Гц.
- Угловая сетка — 50 ячеек по 799.5 мкс (7.2°). Из них 48 несут обычное ребро,
  а нулевая метка занимает ДВЕ ячейки одним широким импульсом.
- Физических нарастающих фронтов за оборот ровно **49** (48 рёбер + метка) —
  в 152 оборотах из 154 (в `encoder8.vcd` два оборота 48 и 50 из-за случайного
  дребезга). Это согласуется с `ENC_RIBS = 50`: 49 реальных + 1 виртуальная
  (`virtual_rib_detected_stb` в `ref_encoder_v4.sv`) на месте второй ячейки метки.
- Обычное ребро: высокий уровень 292 мкс, пауза 493 мкс (скважность ~37 %).
- Нулевая метка: 1031…1034 мкс высокого уровня, и это ровно **2.580 % периода
  оборота во всех четырёх записях** — то есть ширина метки задана геометрией
  диска, а не временем. Комментарий «996 мкс» в `raw_encoder_from_file.sv` верен
  только для своей скорости (996 мкс ⇒ оборот 38.6 мс ≈ 25.9 Гц); сравнивать
  метку надо в долях оборота, а не в микросекундах.
- Окрестность метки воспроизводится с точностью до микросекунд (медиана по всем
  оборотам): … 359, 351, 280, 275 мкс — пауза 569 — **МЕТКА 1031** — пауза 565 —
  **109**, 240, 290, 290 мкс. Первое ребро после метки втрое короче нормы, два
  ребра перед меткой тоже укорочены. Это норма, а не дефект.
- Ложных Z укороченные соседи не дают: `ref_encoder_z_rib_detector` сравнивает
  ПЕРИОД (`instant_period_cnt >= 1.75·rib_period`), а не ширину импульса, и только
  в бо́льшую сторону. Период через метку 1596 мкс = 2.0 штатных шага — порог 1.75
  проходится с запасом.

**Два постоянных дефекта диска — рёбра #8 и #32 от нулевой метки.**

Разорваны на КАЖДОМ обороте всех четырёх записей (162 и 156 событий на ~154
оборота). Структура слота (медиана, «+» высокий / «−» низкий):

| ребро | угол от метки | структура слота | нарастающих фронтов |
|---|---|---|---|
| #8  | ≈57.6°  | +190 −72 **+12** −584 мкс | 2 (интервал 262 мкс) |
| #32 | ≈230.4° | +148 **−15** +162 −477 мкс | 2 (интервал 163 мкс) |

**Ловушка порога.** На диске есть НАСТОЯЩИЙ узкий зуб шириной 56.7 мкс на
угле ротора 151.8° (при норме 292 мкс). Любой фильтр «выкинуть импульсы
короче 100 мкс» его съедает, и запись получает ложный «выпавший зуб» —
второй длинный промежуток за оборот. В самой записи длинный промежуток
ровно один во всех 11 оборотах, зуб на месте. Порог давить только дребезг:
не выше 3 мкс.

Всё остальное — случайный шум: 25…39 дребезгов фронта (1…3 такта) на запись,
любая другая позиция ребра встречается 2…5 раз на 154 оборота. **Внутри самой
нулевой метки провалов нет ни разу ни в одной записи.**

Итог по сырому потоку: за оборот приходит не 49, а **51 нарастающий фронт**
(мода; хвост 52…56 от случайного дребезга) — ровно +2 от постоянных дефектов.

**Что делают эти дефекты с углом — проверено симуляцией (20.08.2026).**

Прогон ModelSim по коммиту `bcfb26da` (тракт энкодера в рабочем дереве
побайтно совпадает с ним, `git diff` по `src/ref_encoder`, `src/TDC/encoder_processing`,
`src/MCU_FPGA`, `src/math`, `src/machines` пуст). DUT — `angle_calculator`
(`ENC_1` → `ref_encoder_v4` + `angle_360_to_mirror_180`), `LEAD_TAU_TICKS = 1700`
как в `encoder_processing.sv`, `in_angl_offs_mgrad = 72500` (регистр 1176).
Стимул — запись `encoderlast.vcd`, срезанная от метки до метки на 12 оборотов и
переведённая в RLE (тик VCD = 20 нс = такт 50 МГц, пересчёта нет). Контрольный
прогон — тот же стимул с заделанными разрывами щелей.

- **Ранняя гипотеза «лишние рёбра → `tooth_err_f` → уход на 14°» НЕВЕРНА.**
  Симуляция: `real_rib_detected_stb` = 50.02 события за оборот в ОБОИХ прогонах,
  Z-детект 1 за оборот, `z_rejected_stb` = 0, `tooth_err_f` срабатывает один раз
  за оборот и в контрольном прогоне тоже (это штатный перенос счётчика на
  50-м ребре, а не признак неисправности). `enc_low` (IIR Баттерворта 2-го
  порядка + гистерезис) лишние фронты как СОБЫТИЯ гасит.
- **Реальный механизм — сдвиг фазы, а не счёта.** Разрыв щели тянет выход IIR,
  порог гистерезиса пересекается не там, и момент ребра уезжает. Ошибка угла:
  ступень +6.16° (ребро #8) и +6.19° (ребро #32) в кадре 0..180°, дальше
  медленно рассасывается к следующей Z.
- **Ни один существующий сторож этого не видит**: окно валидации Z, гейт
  свежести и `tooth_err_f` молчат. Дефект проходит тракт как чистая угловая
  ошибка.
- **Контроль доказывает причину**: с заделанными щелями остаточное отклонение
  0.09…0.27° (медиана по граням, угол ротора), потолок ложится ровной линией.
  Весь наклон и оба разрыва вызваны именно этими двумя щелями.

При `angl_offs_mgrad = 72500` дефекты раскладываются по граням так
(грань = `out_mirror_num` из ПЛИС, 10.9 оборотов после захвата):

| грань | ошибка угла в кадре, ° p50/макс | промах по потолку 3 м, м p50/макс | разрыв |
|---|---|---|---|
| 0 | 1.82 / 3.10 | 0.081 / 0.214 | — |
| **1** | **4.53 / 5.59** | **0.220 / 0.267** | **1.43°** |
| 2 | 1.28 / 2.80 | 0.072 / 0.189 | — |
| **3** | **4.66 / 5.58** | **0.268 / 0.322** | **1.48°** |

Ребро #8 (ротор 57.6°) ПЛИС видит на грани 1, угол в кадре 41.2…58.3°;
ребро #32 (ротор 230.4°) — на грани 3, угол в кадре 28.4…45.8°. Пересчёт на
другое смещение: `грань = floor(((ребро·7.2° + смещение) mod 360) / 90)`, но
отсчёт ПЛИС сдвинут относительно начала метки на ≈17.4° (первое ребро после
двухъячеечной метки плюс фаза ФАПЧ) — при другом смещении проверять прогоном,
а не формулой.

**Что из этого увидит ПО (прогон всего `encoder_processing`, 20.08.2026).**
Тело MSOP несёт углы неявно: точка k рисуется на `first_angle + k*angle_res`
(комментарий `MSOP_TCP_frame_tracker.sv`), дыра добивается невалидными узлами,
точка на пройденном узле отбрасывается (`msop_tcp_pipeline`). Режим боевой:
смещение 72.5 град, разрешение 0.2 град, сектор 30...150 град; 12 оборотов,
28584 выстрела, 56 смен грани, Errors 0. За 11 оборотов — 44 пакета, 26213 точек.

| кадр грани | точек | добито невалидных | ошибка угла p50 | макс |
|---|---|---|---|---|
| 0 | 6380 | 0 | 1.85° | 4.24° |
| **1** | 6611 | **44** | **4.10°** | 4.88° |
| 2 | 6611 | 0 | 1.59° | 3.77° |
...
Контроль с целыми щелями: невалидных узлов 0, ошибка угла p50 0.38°, макс 1.24° —
это точность самого разбора. Два эффекта видны только на уровне MSOP: якорь
`first_angle` берётся раз на кадр, поэтому ошибка в его момент сдвигает ВЕСЬ
кадр; и каждый дефект съедает 4 узла за кадр (0.8° тела) невалидными точками —
дырка в облаке. Полный разбор с картинками — заметка ваута «Дефекты диска
энкодера R120M — вклад в угол по симуляции 20.08.2026».

Артефакты: `C:\Users\User\Downloads\encoder_potolok_72_5.png`; скрипты прогона
и разбора — во временной папке сессии (ТБ `enc_ceiling_tb.sv`, плеер RLE,
`run_enc.do`), в репозиторий не клались.

## Сессия на железе: угол по нулевой метке, 192.168.2.206 (20.08.2026)

Ветка `R120M.BF2_TDC7201_zmark` от `bcfb26da` (вершина боевой линии TDC7201),
worktree `C:\workspace\verilog_zmark`. Коммиты `d161ecf9` (RTL) и `b4cdeb8c`
(результаты). Проектная заметка — `docs/zmark_angle.md` в этой ветке.

- Плата R120M.BM2BF1X1, топ `R120M_BM2BF1X1_brd_ifm`, сборка
  `build_r120m_bm2bf1x1.tcl`, GW2A-18C.
- Образ `R120M.BF2_TDC7201_zmark.git-d161ecf97e94.img-1b4232b5d3f3`, сырой
  `.bin` 907 418 байт.
- Загружённая конфигурация на момент замера: `angle_res` = 360 мград (дефолт
  после заливки), `distance_byte_count` = 3, `intensity_flag` = 1,
  `echo_mode` = 3, `echo_count` = 2, мотор 600 об/мин, сектор полный.
- До заливки в приборе стоял `R120M.BF2_TDC7201_encdiag.MF6FP5X2` (образ
  `316c60eebf42`). Снят полный дамп флэша 4 МиБ —
  `releases/backup_206_before_zmark.bin` в worktree.

**Площадь (тот же проект, только источник угла):**

| | Logic | Register |
|---|---|---|
| прежний тракт (ФАПЧ + зубья) | 12460/20736 60 % | 11138/16173 68 % |
| угол по нулевой метке | 10956/20736 52 % | 9554/16173 59 % |

**Измерено на живом приборе**, поток MSOP 10 с непрерывно, 400 кадров:
кадров 39.99 в секунду = 9.998 оборота/с при заданных 600 об/мин; точек в кадре
500 во ВСЕХ 400 кадрах (500 × 0.36° = ровно 180° грани); `error_code` = 0 везде;
прирост `frame_number` = +1 в 100.00 % переходов. Обратное чтение флэша после
заливки совпало байт в байт: 907 418 из 907 418, хвост 0xFF.

Точки выходят только при `out_enc_fresh` = 1 (гейт в `encoder_processing`),
поэтому непрерывный поток сам по себе означает, что тракт захватился и держится.

**Отвергнутые варианты и число, которым они отвергнуты** (все три умерли о
прогон ModelSim, каждый выглядел правдоподобно):

- ограничивать фазовый накопитель на 360° — ложное «метка опоздала» каждый
  оборот, 8 срабатываний за 12 оборотов;
- признак метки `ширина > T >> 6` — самораскрутка порога: T уехал
  842k → 681k → … → 120k тактов вместо 2 000 846;
- принимать период с одного интервала — вторая «метка» на диске уводит T куда
  угодно; нужны два совпавших интервала подряд.

**Грабли заливки (дороже всего):**

- **`--reply-timeout 5000` из `docs/enc_edge_diag.md` мало.** Прибор отвечает до
  10 674 мс; первая заливка сорвалась на 12 % (блок 61 из 507), флэш остался
  стёртым и частично записанным. Повтор с `--reply-timeout 30000` прошёл
  полностью. Прибор при срыве остаётся живым: пингуется, порт загрузчика 50102
  открыт, перезаливка идёт сразу — паниковать и лезть программатором не нужно.
- **Прибор односеансовый по каждому порту.** Последовательная проверка
  50100/50101/50102 оставила полузакрытые сокеты `FIN_WAIT_2`, и следующая
  команда получала `Connection refused` около минуты. Порты не сканировать.
- **Открытый GUI `lconf` держит 50100/50101**: ни дамп, ни заливка не идут.

**Разбор потока MSOP без GUI.** Заголовок 36 байт по
`src/lidar/package_lidar_pack_MSOP_TCP_1.svh`, хвост `FF 9B`, длина кадра
`36 + N·8 + 2` при `echo_count=2` и `distance_byte_count=3`. Многобайтные поля
идут МЛАДШИМ байтом вперёд — из описания структуры это не следует, проверено на
живом потоке (`point_count` = 0x01F4 = 500 читается только так, и только тогда
длина кадра сходится с наблюдаемыми 4038 байтами).

## Штатные пресеты сканирования и ловушка ACK (20.08.2026)

Пресеты живут в ПО (`lconf/mainwindow_tab_control.cpp`, заполнение `presets_`) —
пары «частота кадров / угловое разрешение»:

| Гц | ° | Гц | ° | Гц | ° | Гц | ° |
|---|---|---|---|---|---|---|---|
| 200 | 0.4 | 160 | 0.36 | 150 | 0.3 | 120 | 0.24 |
| 200 | 1.2 | 160 | 0.9 | 150 | 0.9 | 120 | 0.72 |
| 100 | 0.2 | 80 | 0.18 | 50 | 0.1 | 40 | 0.08 |
| 100 | 0.6 | 80 | 0.45 | 50 | 0.3 | 40 | 0.24 |

Частота кадров = 4 × оборота/с (четыре грани), поэтому 40 Гц — это 10 об/с.

Задаётся без GUI:

```
lconf.exe --lidar-control --control-channel tcp --control-host <IP> \
          --control-port 50101 --control-timeout 8000 \
          --set-scan-mode --angle-res 0.24 --frame-rate 40
```

**ACK — эхо ПРЕЖНЕГО значения, а не подтверждение применения.** На запрос
`ANG_RES 0.24 deg` прибор ответил `ACK: 5000 mdeg` (то есть 5°, что стояло до
команды), и по этому ответу легко решить, что команда не прошла. В потоке при
этом `angle_res` стал 240 мград, а `point_count` — 583 (сектор 20.16…159.84° при
0.24°: 140/0.24 = 583). Проверять применение только по заголовку кадра.

После заливки прошивки настройки ПЛИС сбрасываются в дефолт (0.36°, полный
сектор), поэтому пресет надо задавать заново каждый раз.

## Файл ограничений у Gowin ОДИН, и лишние отбрасываются молча (21.08.2026)

Самый дорогой урок этой недели. Отказ выглядел как мёртвая ПЛИС, а был ошибкой
сборки, о которой сборка не сказала ни слова.

### Что происходило

Четыре вывода SPI2 были вынесены в отдельный файл рядом с вариантом отдачи кадра
MSOP — разумно по смыслу: `DRIVE` и `SLEW_RATE` законны только для выходов, а при
переходе на очередь три вывода из четырёх становятся входами. В скрипт сборки оба
файла добавлялись подряд:

    add_file .../src/main/R120M_BM2BF1X1.cst
    add_file .../src/main/msop_transport/master/R120M_BM2BF1X1_spi2.cst

**`gw_sh` читает не все добавленные файлы ограничений, а последний.** В журнале
сборки строка `Reading constraint file` ровно ОДНА, и это вариантный файл.

### Чем это кончилось

Основной файл не читался вовсе, и синтезатор расставил все прочие выводы сам.
Снято с отчёта `impl/pnr/*.pin.html`:

| цепь | должна быть | встала |
|---|---|---|
| SSPI_CMD_SCK  | C10 | H5  |
| SSPI_CMD_MOSI | T10 | T11 |
| SSPI_CMD_MISO | M8  | J6  |
| SSPI_CMD_CS_N | N9  | N10 |
| FLASH_SPI_SO  | P10 | K3  |
| FLASH_SPI_SI  | R10 | H4  |
| FLASH_SPI_CS  | M9  | R1  |
| FLASH_SPI_CLK | L10 | K6  |

Верно встали только выводы SPI2 — из единственного прочитанного файла.

### Почему разбор ушёл не туда

Образ собирался **без единого замечания**, заливался и сходился при обратном
чтении. На приборе это выглядело как «ПЛИС не поднялась»: обмен по SPI3 давал
сплошные единицы, опознание микросхемы через проброс — `0x00FFFFFF`, прошивка МК
честно уходила на спасательный путь SSPI.

**Признак «проброс мёртв» НЕ означает «образ не загрузился».** Проброс живёт
внутри образа и молчит в двух разных случаях: образа нет, и образ есть, но в нём
сломан командный канал. Второй случай здесь и был, а разбор полдня шёл по первому
— искали негодный битстрим, порчу флеши, цепочку многозагрузки.

### Правила

- **Файл ограничений один — и в дереве, а не только на входе сборки.** Разделять
  на «основной плюс вариантный» нельзя: части не складываются, поздний заменяет
  ранний.

  Первым лечением была склейка частей во временный файл перед добавлением. Она
  работала, но оставляла в дереве два источника правды, и её отменили по прямому
  указанию: «не нужно делать разные cst — только один». Склейка чинит симптом и
  сохраняет причину — возможность собрать образ из не той половины.

  Стоимость единственного файла — в том, что варианты сборки, которым нужны
  ДРУГИЕ свойства выводов, приходится закрывать. У R120M.BM2BF1X1 так и вышло:
  прежний путь отдачи кадра (ПЛИС ведущая) снят вместе со своим файлом. Он был
  мёртв — прошивка МК ведущая на этой шине с выпуска R120M.BM2MF8MP2X5, — а
  сборка, которую нельзя залить, существует ровно затем, чтобы однажды быть
  собранной по ошибке.

  Сведение проверяется ПОБАЙТНОЙ сверкой битстрима: тело `.fs` до и после
  обязано совпасть посимвольно, различаться вправе только строка `//Created
  Time`. Совпало — значит переложили, а не переделали.
- **Проверять расстановку выводов ДО заливки.** Отчёт `*.pin.html` есть после
  каждой сборки, чтение занимает секунду и ловит именно такие отказы. Стоимость
  пропущенной проверки — цикл заливки, отказ и разбор по ложному следу.
- **Одна строка `Reading constraint file` на сборку — это норма, а не подсказка.**
  Считать их и сверять с числом добавленных файлов.

## ПЛИС исполняет образ по 0x200000, а не по нулевому адресу (21.08.2026)

Патч перехода в заголовке образа по адресу 0 уводит на золотой **безусловно**, а
не при отказе. То есть половина, которую заливщик называет «рабочей», не
исполняется вовсе: она служит указателем.

Доказано опытом с различимыми образами: в рабочую половину положен заведомо
годный образ комплекта, в золотую — образ с опознавательным признаком. После
перезагрузки ПЛИС отвечает признаком, то есть исполняется 0x200000.

Раньше это не проявлялось, потому что `fpga_golden.py` кладёт ОДИН И ТОТ ЖЕ образ
в обе половины: какая бы ни исполнялась, поведение одинаково. Ошибка пряталась
ровно за тем, что делало её безвредной.

**Следствие: запасного хода в этой схеме нет.** Обе половины несут один образ, и
негодный образ делает негодными обе. Спасает только SSPI — путь, не зависящий от
образа; он проверен и в этот день сработал трижды.

Прежний опыт `fpga_chain_check.py` этому не противоречит: там обе половины тоже
несли один образ, и все пять шагов объясняются «исполняется золотой» так же
хорошо, как «подхватывает при отказе». Опыт не различал эти две гипотезы.

## Диагностический ответ обязан отличаться от молчания (21.08.2026)

Счётчики тракта были положены по адресу 1400 — ВНЕ массива ячеек (`MEM_BYTES =
500`). Образ без этой ячейки отдаёт по такому адресу нули. Образ с ячейкой, пока
счётчики нулевые, отдаёт те же нули.

Два разных исхода одним видом: «счётчики стоят» и «отвечает другой образ»
неразличимы. На этом разбор и застрял: нули читались как «сигнала нет», а
означали «отвечает не тот образ».

Лечится не аккуратностью, а признаком. Ячейка перенесена на адрес 480 — ВНУТРЬ
массива, где образ без неё отдаёт записанное, то есть ЭХО, а образ с ней — магию
`'PK'`. Один обмен закрывает вопрос «что исполняется» положительным ответом.

Побочный урок: промежуточный массив байтов, заполняемый в `always_comb`,
синтезатор волен уложить в блочную память, а её нельзя читать комбинационно.
Байт состояния выбирается срезом с базой из четырёх разрядов
(`in_status[{data_cnt[3:0], 3'b000} +: 8]`) — обычный мультиплексор.

## Кадр, не влезающий в пакет, ЗАКЛИНИВАЛ тракт измерений (21.08.2026)

Двенадцать режимов из шестнадцати не давали ни одного кадра, хотя обороты
вставали на всех и с точностью в сотые доли герца. Выглядело как «поток не идёт
на низких оборотах» — и это было неверное описание.

### Опыт, который отделил потерю от заклинивания

Три шага на ОДНОЙ скорости, меняется только угловое разрешение:

| шаг | разрешение | точек в кадре | кадров за 5 с |
|---|---|---|---|
| 1 | 0.9° | 200 | 719 |
| 2 | 0.3° | 600 | 0 |
| 3 | 0.9° (возврат) | 200 | **0** |

Третий шаг и есть весь смысл опыта. Одиночная потеря пакета переживается;
здесь тракт остаётся мёртвым до перезагрузки ПЛИС. Без третьего шага отказ
описывался бы как «мелкое разрешение не работает», и чинили бы не то.

**Правило: в любом опыте с отказом обязателен шаг ВОЗВРАТА к заведомо рабочему
состоянию.** Он отличает потерю от заклинивания, а это разные болезни.

### Корень

`MAX_PACKET_POINTS = 4500` — число, не связанное ни с чем. При точке в восемь
байт (два эха по [dist(3)+int(1)]) это 36 килобайт, вчетверо больше того, что
принимает транспорт: очередь ПЛИС и приёмник МК держат 4096 байт.

Предел для того и заведён, чтобы лишние точки ОТБРАСЫВАЛИСЬ до нового кадра. С
недостижимым значением он не срабатывал никогда.

Числа с прибора: `too_long` 247, `max_len` 4091 (наибольший принятый пакет),
`hdr_bad` вырос с 1 до 107 — переросшие кадры не только терялись, но и сбивали
разбор рамки.

### Как чинилось

Предел берётся из той же функции, что и граница стоячего пакета: по бюджету
транспорта и ДЕЙСТВУЮЩЕЙ настройке точки. Параметр модуля сохранён как
переопределение для стендов (ноль — считать, ненулевое — принудительно).

После правки тот же опыт: 719 / 295 / 730. Прогон всех шестнадцати режимов —
приняты все, поток идёт на каждом, сбоев ноль.

### Цена, которую надо знать

На мелком разрешении кадр ОБРЕЗАЕТСЯ до 507 точек — столько влезает в пакет при
двух эхах и трёх байтах дистанции. Запрошенные 2250 точек на 0.08° прибор не
отдаст, пока кадр не научится дробиться на несколько пакетов. Это следующий шаг;
обрезание всяко лучше заклинивания, но молчать о нём нельзя.

## Замер, снятый с заклиненного прибора, врёт уверенно (21.08.2026)

После заливки исправленного образа опыт дал ноль кадров на ВСЕХ трёх шагах,
включая заведомо рабочий. Вывод напрашивался: правка сломала и то, что работало.

Вывод был неверен. ПЛИС оставалась заклиненной с прошлого прогона, а
перезагрузка, которую я сделал перед заливкой, к делу не относилась: заклинило
её уже ПОСЛЕ, первым же шагом опыта. Проверка стоячего потока сразу после
перезагрузки дала 170,5 КиБ/с — то есть правка была цела, а негодным был замер.

**Правило: перед опытом приводить прибор в заведомо известное состояние и
УБЕЖДАТЬСЯ в этом отдельным измерением.** «Я только что перезагрузил» — не то же
самое, что «сейчас измерено, что состояние чистое».

## Номер кадра рос через пакет: три копии одной величины (21.08.2026)

Программа на компьютере показывала 49,99 % потерь при полном их отсутствии:
прибор отдавал 895 кадров из 895. Клиент читает номер кадра как сквозной счётчик
пакетов, а повтор номера считает дубликатом (`data_reader_base.cpp`, ветка
`cur == last`).

Сколько точек влезает в стоячий пакет, считали ТРИ места и все три по-разному:

    отправитель  по union тела, 8 байт  -> 123 точки
    конвейер     по 3 + 1 = 4 байта     -> 246
    стенд        своя localparam, 4     -> 246

Резал пакеты первый, номер увеличивал второй — два пакета на один номер. Стенд
не ловил этого потому, что сверял каждую сторону сам с собой по третьей копии.

**Правило: величина, от которой зависят обе стороны, обязана иметь ОДИН
источник. А стенд обязан сверять стороны МЕЖДУ СОБОЙ, а не каждую с собственной
копией истины.** Добавленный сторож сверяет именно это, и его «пустоту»
проверили нарочно: с прежним расчётом он даёт «конвейер=246 трекер=328».

Размер точки — пользовательская настройка (байты дистанции и число эх), поэтому
величина считается во время работы, а не при сборке. Побочная польза: при одном
эхе и двух байтах дистанции пакет берёт 328 точек вместо прежних 123.

## Перенастройка ПЛИС без перезапуска МК ломает поток (21.08.2026)

Отказ выглядит как мёртвая ПЛИС и ею не является. После команды `0x45`
действие 5 ПЛИС поднимается с УМОЛЧАНИЯМИ, а настройки ей выдаёт МК ОДИН РАЗ
при своём запуске (`FpgaTable_ApplyAll`, `Config_Init` — до входа в цикл).
Никто их не переслал, и дирижёр перестаёт разрешать выстрелы.

Как это читается по приборам:

| что видно | значение |
|---|---|
| `device_check`: всё «ок», кроме «поток измерений» — таймаут | поток не идёт |
| диагностическая ячейка: «выстрелов разрешено 0» | дирижёр не пускает |
| «привод стоит True», фронтов энкодера 0 | это НЕ причина при стоящем двигателе |
| проброс к флеши жив, отпечаток сходится | образ загрузился и работает |

Последняя строка и есть ловушка: образ ЖИВ, а поток мёртв, и на «ПЛИС не
поднялась» это не похоже совсем. Я успел записать отказ на свою правку RTL
прежде, чем проверил простое.

**Порядок обязателен: залил образ -> перенастроил -> ПЕРЕЗАПУСТИЛ МК.**
Проверка — сброс МК и повторный `device_check`; поток возвращается сразу.

Смежное правило из той же семьи: после заливки прошивки МК настройки ПЛИС
тоже сбрасываются в умолчания, поэтому пресет задаётся заново каждый раз.

## Регистры настроек не читаются обратно — доказывать надо эхом (21.08.2026)

Адреса от 1000 и выше защёлкиваются в регистры ПЛИС и при чтении отдают нули.
Значит удачный обмен подтверждает РАЗБОР РАМКИ, но не срабатывание ветки
нужного адреса, а обратное чтение не подтверждает ничего.

Разрыв закрывается эхом в диагностической ячейке: у лидара регистр `LEDS`
(адрес 1080) выведен в байт 3 ячейки 480, бывший резерв. Прибор сам
докладывает, что́ у него защёлкнуто, и это читается по сети.

Приём общий: **у любого регистра «только на запись», от которого зависит
работа, должно быть эхо в читаемой ячейке.** Иначе единственный способ
проверить — светодиод или осциллограф, то есть глаза на месте.
