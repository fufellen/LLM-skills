...
`Дист., байт` 2 → 3 → 2 в ПО. Это дёргает `lcl_cfg_apply_stb` (до
переименования — `pack_format_flush_stb`) — ЕДИНСТВЕННОЕ место в
`MSOP_TCP_sender`, сбрасывающее указатели body RAM.
...
## Паспорт энкодера: угол по измеренной геометрии диска — HWOK (07.08.2026)

Ветка `R120M.BF2_enc_passport` (от `c364662b`), изделие `192.168.2.241`
(LTDC, MAC 00:80:E1:3B:00:10). Setup/Hold = 0, CLS 91% (было 88%).

Суть: угол считался как `7200·rib_cnt + 7200·precise/period` — то есть в
предположении, что все 50 слотов ФАПЧ ровно по 7.2°. Модуль `enc_passport`
меряет фактическую длительность каждого слота, нормирует её на время оборота
(при постоянной скорости угол пропорционален времени) и копит нормированные
ширины в таблице. Угол стал `phi_base[слот] + phi_span[слот]·precise/period`.

Симуляция (диск с неравномерностью 1% зуба, `enc_passport_tb`): размах
ошибки угла внутри оборота 1678 мград ВЫКЛ против 37 мград ВКЛ — **в 45 раз**.
Вывод оттуда же: ФАПЧ с `FEEDBACK_COEFFICIENT = 4` тянется за каждым зубом
почти целиком, поэтому геометрия диска уезжает в угол практически полностью.

Замеры на железе (три прошивки с ОДНОЙ ветки, различие только параметром):
- шумовой пол метода: два подряд захвата на одной прошивке — **0 мград** по
  граням 0 и 2 (грани 1 и 3 на этом стенде дают мусорную подгонку, невязка
  на два порядка выше — считать только по 0 и 2);
- ВПРЫСК известной таблицы (`ENC_PASSPORT_LEARN=0`, `INJECT=576`, треугольник
  ±0.5% слота): предсказание — сдвиги противоположных граней в сумме дают
  1800 мград (треугольник антисимметричен через полоборота); измерено
  +250 и +1450 = **1700** мград. Тракт паспорта доезжает до точек, множитель
  верный;
- ВЫКЛ → ВКЛ (обученная таблица): грань 0 −860, грань 2 −1500 мград;
- повторяемость ВКЛ: 0 мград между двумя захватами;
- независимость от скорости: 160 → 40 → 160 Гц возвращает картинку в
  0 / −20 мград (нормировка на оборот работает, таблица не портится);
- регрессии нет: 160 Гц, узлов сетки 699/684 против 696/681 на ВЫКЛ.

Что НЕ доказано: что картинка стала ТОЧНЕЕ. Без внешнего углового эталона
доказуемы только механизм, повторяемость и независимость от скорости.
Угловая структура поправки внутри кадра выше шума (MAD 290 против 65-80
мград), но выделить в её спектре шаг зуба (14.4° по углу кадра) не удалось —
сильнее компонента на 25-27°, вопрос открыт.

### Ловушка: гейт свежести рёбер проваливается КАЖДЫЙ оборот

`out_enc_fresh` штатно снимается на ~3 такта на широкой Z-метке: её пауза
лежит ровно на пороге детектора разрыва (`FRESH_GAP_PERIODS = 2`, а зуб Z
как раз 2 периода). Первая версия сторожа «оборот грязный, если свежесть
пропадала хоть на такт» намертво заблокировала обучение — `good_revs` не
поднимался выше нуля, и прошивка ВКЛ вела себя ровно как ВЫКЛ.

Поймано симуляцией (`enc_passport_diag_tb` печатает гейты на каждом обороте)
ДО заливки — то есть попытка в железе не потрачена. Лечится допуском:
считать такты без свежести и сравнивать с 1/256 оборота; настоящий разрыв
длится не меньше пары слотов и порог перешагивает.

Правило: гейт «ни одного провала за оборот» на этом железе не работает
никогда. Любой сторож по `out_enc_fresh` должен иметь порог.

### Стенд .241: на 90 Гц поток почти пустой, на 160 Гц полноценный

На кадровой 90 Гц МК отдаёт 123 точки в пакете, кадр покрывает всего 22-44°,
и валидных точек 0.25%. На 160 Гц — 250 точек в пакете, кадр 20..160°,
валидных 45%. Прежде чем объявлять «лидар ничего не видит», поставить 160 Гц.
Разрешение 200 мград (0.2°) при 160 Гц даёт ~700 узлов на грань при полном
охвате — это рабочий режим для угловых замеров.

После каждой заливки: `speed 160`, затем `res` — МК свои настройки в ПЛИС
не восстанавливает (дефект 2253).
