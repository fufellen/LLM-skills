...
## CFG0 бит 2: даташит требует «Set 1», и это ОШИБКА — чип перестаёт измерять

**Что написано в документе.** Table 8 (стр. 13, Production Rev. 2) для `CFG0`
бит 2 прямо предписывает `Set 1. reserved`. То же было и в Preliminary. Внешние
разборы даташита ссылаются на это как на пункт, который надо исправить в
прошивке, и реконструируют «правильные» значения `CFG0` = `0x07 / 0x17 / 0x27`.

**Что на самом деле (проверено на железе 13.08.2026, чередованием).**

| `CFG0[2]` | `CFG0` при HIGHRES=2 | Результат |
|---|---|---|
| 0 | `0x23` | работает: ~25 000 отсчётов за захват, среднее повторяется в пределах 2.6 пс |
| **1** | `0x27` | **измерений НЕТ вовсе** |

При установленном бите отказ выглядит обманчиво: конфигурация проходит
контрольное чтение (`cfg_error = 0`), чип рапортует `DEV_STATE = 3`, то есть
откалиброван, — а линия `INTERRUPT` намертво прижата в ноль и не приходит ни
одного результата. Воспроизведено дважды на независимых сборках, возврат бита в
ноль немедленно восстанавливает работу.

**Косвенное подтверждение:** эталонная конфигурация самого вендора
(ScioSense Dashboard, `Ltdcx3.cfg`) даёт `CFG0 = 0x43`, где бит 2 тоже НОЛЬ.
То есть предписание в тексте даташита противоречит и собственному эталонному
конфигу производителя, и поведению кремния.

**Вывод: бит 2 в `CFG0` оставлять нулём.** В обработчике он под параметром
`CFG0_RSVD_BIT2` (по умолчанию 0) — чтобы проверку можно было повторить, а не
принимать на веру ни даташит, ни этот вывод.

## EN_QUAD пишется ПОСЛЕДНИМ — иначе валится сверка конфигурации (железо 13.08.2026)

**Грабля.** `EN_QUAD` живёт в `COM_SET` (0x13), а этот регистр стоит в середине
конфигурационной последовательности: в `ltdc_x3_spi_hdlr` он попадает в блок 1
(`0x11..0x14`), после которого пишутся ещё блоки `0x2A..0x33` и `0x3A..`, а затем
идёт полная сверка контрольным чтением. Стоит включить quad в общем потоке —
и **всё, что однобитный мастер пишет и читает после этого регистра, перестаёт
сходиться**: чип уже сменил режим интерфейса.

**Как это выглядит на плате.** Не «quad не читает», а гораздо неприятнее:
- `cfg_error` дёргается, автомат уходит в повторы конфигурации;
- выгрузка по UART вырождается в ОДИН повторяющийся байт (декодер при этом
  показывает разные константы `0xa4`/`0x1a`/`0x23` — это одна и та же
  10-битная посылка, пойманная в разной фазе);
- число выстрелов падает примерно в 30 раз;
- timing при этом чист (0 setup, 0 hold), симуляция проходит, а 1-битная
  вычитка с той же обвязкой работает — то есть все привычные подозреваемые
  чисты, и на них уходит время.

**Диагностика за одну сборку.** Не гадать, а вывести на отладочный разъём
`quad_busy` и `cfg_error` (в минимальном топе это параметр `DBG_QUAD`). Ответ
читается сразу: `quad_busy` нормально импульсит → мастер вычитки ни при чём;
`cfg_error` не ноль → дело в конфигурации. Гипотеза «залип мастер из-за
константного `rst_n`» этим сразу снимается.

**Решение.** Писать разрешение quad ПОСЛЕДНИМ, отдельной транзакцией, уже после
записи и сверки всей остальной конфигурации, и **не сверять его**: сверка шла бы
уже в изменившемся режиме. В `ltdc_x3_spi_hdlr` это состояния
`S_QUAD_ON → S_QUAD_ADDR → S_QUAD_DATA → S_QUAD_DONE`, а `cfg_data[18]`
(COM_SET) в основном потоке уходит нулём.

Результат: `q=1`, `cfg=0`, `int=1000 to=0000 h1=1000`, σ в обычном диапазоне.

**Общее правило, шире этого чипа.** Регистр, который меняет РЕЖИМ ИНТЕРФЕЙСА,
нельзя писать в середине пакета с последующей сверкой — ни на этом чипе, ни на
конфигурационной FLASH (там та же форма грабли: заголовок пишется последним,
иначе окно обновления оставляет кирпич). Порядок всегда один: сначала всё
содержимое и его проверка, режим — последним действием.
