...
## Полного программного сброса чипа НЕТ. power_up (0x30) его не даёт

**Проверено на железе 14.08.2026.** Чип нельзя вернуть к внутренним умолчаниям
командами по SPI. Единственное, что гарантированно чистит состояние, —
**сброс питания платы**.

**Что говорит даташит, §7.5.1:**

> «After the last bit of the opcode is sent the reset remains active **until SSN
> goes high** (tpor). After the reset ... the configuration registers are **set
> to internal defaults** of the chip.»

Читается так, будто сброс держится, пока SSN прижат, и заканчивается его
подъёмом. Отсюда напрашивается правка: удерживать SSN после опкода `0x30`.

**ГИПОТЕЗА ПРОВЕРЕНА И ОПРОВЕРГНУТА.** Реализовано удержание SSN (`por_hold` в
обработчике, мукс в интеграции), испытано в двух вариантах:

| Вариант | Результат |
|---|---|
| SSN прижат весь `POWER_ON_DELAY` (10 мс) | `cfg_error = 1`, `DEV_STATE = 0`, измерений нет — 3 прогона из 5 |
| SSN прижат 1 мс, затем отпущен, потом выдержка | то же самое |
| SSN не прижимается (как было) | работает |

Переноса состояния между перепрошивками удержание тоже НЕ устранило. Правка
оставлена параметром `POR_SSN_HOLD` со значением по умолчанию **0**, чтобы
гипотезу не переоткрывали.

**Хуже того:** сами эти опыты загнали чип в состояние, из которого он не вышел
ни одиночной, ни тройной перепрошивкой ПЛИС — только сбросом питания. Это и
есть прямое доказательство, что программного полного сброса у нас нет.

**Практические следствия.**
- Смена конфигурации чипа на ходу требует ДВУХ прогонов пусковой
  последовательности (см. ниже, START_CYCLES = 2). Это работает и решает
  задачу; полного программного сброса при этом всё равно нет.
- Если чип встал (`cfg_error = 1`, `DEV_STATE = 0` и перепрошивки не помогают)
  — **передёрнуть питание**, других вариантов нет.
- Для боевой прошивки не блокирующее: там конфигурация не меняется на ходу, а
  при холодном старте чип поднимается чистым.
- Открытый путь, не проверенный: аппаратный сброс питания чипа с ПЛИС либо
  линия `REINIT` в связке с чем-то ещё.

**Общий урок:** прежде чем чинить по букве даташита, проверять правку
чередованием — формулировка «reset remains active until SSN goes high» выглядит
однозначной, а на кремнии удержание SSN делает только хуже.

## Перенос состояния: на ЧИСТОМ чипе одного прогона достаточно (уточнено 14.08.2026)

**Итог расчленяющего эксперимента.** Контрольный одиночный прогон (смена
HIGHRES 1 -> 2, одна прошивка, БЕЗ повторов) на стенде после сброса питания
даёт норму — перенос состояния не воспроизводится. Проверено пятью независимыми
прогонами: контроль, контроль с 40-секундной выдержкой отравляющей прошивки и
три варианта с добавками (tdc_stop перед power_up; power_up дважды; запись
конфигурации дважды) — все здоровы, различий между ними нет.

**Что этим установлено.** Перенос состояния — не свойство чипа как таковое, а
симптом ЗАЛИПШЕГО состояния, в котором чип находился всю марафонскую сессию
13-14.08: все наблюдения переноса (кластеры по предшественнику, пустые HR=0,
+94 пс и +19 % выстрелов) сделаны в одну непрерывную сессию питания, после
серии агрессивных конфигурационных экспериментов. Поскольку полного
программного сброса нет (см. раздел ниже), это состояние пережило десятки
перепрошивок — до передёргивания питания.

**Чем вызывается — НЕ УСТАНОВЛЕНО.** Попытки намеренно вернуть перенос на
чистом стенде провалились: ни CFG0[2]=1 (бит, останавливающий измерения), ни
TR_MODE=2 (чип принимает конфиг, но не калибруется) поодиночке его не вызывают.
Вход в залипшее состояние потребовал чего-то ещё — комбинации, длительности
или конкретной последовательности той сессии.

**Отсюда честная переоценка прежних выводов.** «Вторая перепрошивка лечит» и
«START_CYCLES=2 устранил перенос» были получены БЕЗ контроля с одиночным
прогоном на свежем стенде: после сброса питания одиночный прогон здоров сам по
себе, так что вклад двойного прогона в те наблюдения недоказуем.

**Правила.**
- После сброса питания смена конфигурации работает С ПЕРВОГО прогона —
  специальных мер не требуется.
- `START_CYCLES = 2` оставлен как дешёвая страховка (~15 мс к старту, вреда
  нет) на случай повторного входа в залипшее состояние.
- ПРИЗНАКИ залипания, по которым его ловить: результат зависит от предыдущей
  сборки; число выстрелов за захват раздуто (~+19 %) — темп задаёт задержка
  отклика чипа, зависящая от HIGHRES; среднее сдвинуто на ~+94 пс. Лечение —
  сброс питания.
- В обработчике под эксперименты оставлены параметры `STOP_FIRST`, `PWR_TWICE`,
  `CFG_WRITE_TWICE` (по умолчанию 0) и зеркало SPI `DBG_SPI_MIRROR` в топе;
  `START_CYCLES = 0` крутит пусковую последовательность бесконечно — для
  съёма диалога анализатором без триггера.

**Оговорка по зеркалу SPI:** отладочные ножки в `.cst` намеренно 4 мА SLOW, и
25-МГц SCK через них приходит смятым — для декода эпюр пускового диалога нужна
пересборка с быстрым током на DBG-ножках. Вторая оговорка, дороже первой:
зеркало на E1 глушит наводкой UART на соседнем E2 (проверено 15.08.2026),
то есть включённое зеркало отнимает канал, по которому прошивка отчитывается о
себе. Подробности и правило разноса — в навыке `dsview/dslogic-plus-cli.md`.

**Как отличить мёртвый чип от мёртвого стенда (порядок проверки 15.08.2026).**
Когда минимальный топ встаёт с `dev=0`, соблазн — чинить топ. Дешевле сначала
доказать, что железо живо: прошить БОЕВОЙ релиз и посмотреть, идёт ли поток.
На стенде 15.08 релиз дал MSOP и СКО 65.4 пс, то есть чип, SPI-линии и оптика
были исправны, а не отвечал он именно экспериментальной прошивке. Следующий шаг
столь же дешёвый: собрать ПОСЛЕДНЮЮ ЗАКОММИЧЕННУЮ ревизию своего же топа. Она
поднялась (`dev=3`, СКО 82.5 пс) — значит виноваты несохранённые правки, и
искать надо в них, а не в чипе. Обе проверки занимают по одной сборке и
снимают целый класс ложных гипотез («чип залип», «нужен сброс питания»).

**Две прошивки сравнивать только ЧЕРЕДОВАНИЕМ ПОДРЯД (проверено 15.08.2026).**
Ночь 14–15.08 ушла на поиск причины, по которой боевой релиз давал 58 пс, а
минимальный топ 82–85 пс на одном стенде. Перебраны и опровергнуты девять
гипотез: walk по ширине, темп выстрелов, ширина поджига, токи ножек, таблица
регистров, способ перевзвода (команда `0x19` против импульса на пин), способ
вычитки (quad 50 МГц против однобитной 6.25 МГц), лишняя лазерная линия,
отбор точек по силе отклика, вращение зеркала, смещение каналов.

Причина оказалась не в прошивках. Тот же релиз за ночь прошёл 57.8 → 65.4 →
70.6 → 74–76 пс: **дрейф стенда за ночь превысил все различия, которые
искались в настройках**. При чередовании подряд (цикл в минуту, без
пересборок) релиз даёт 74.4 и 75.6, минимальный топ — 85.4 и 83.0.

Остаток около 9 пс списывать на прошивку тоже нельзя: они меряют РАЗНЫЕ ТОЧКИ
(средние 22 955 против 24 493 пс — цели разнесены на 231 мм), потому что релиз
стреляет по энкодеру в своём секторе, а минимальный топ — непрерывно.

Правило: **разнесённые во времени замеры разброса на этом стенде несравнимы.**
Прежде чем объяснять разницу двух прошивок их кодом, прошей их подряд. И
проверь, совпадают ли СРЕДНИЕ: если нет — это разные цели, и разбросы у них
разные по физике, а не по коду.

**Ложный отказ «чип умер»: виноват был декодер UART, а не прошивка.**
Разбор ночи 14–15.08.2026, который стоил нескольких неверных гипотез подряд.
Захваты снимались на 25 МГц при UART 921600 — формально запас есть (навык
требует минимум 10 МГц), но на практике часть байтов приходила искажённой
одним битом: `st=43` вместо `st=03`, `gfg=0` вместо `cfg=0`, `cur=0s` вместо
`cur=09`. Числа при этом не собирались вовсе, а поле `dev` читалось нулём.

Из этого родились и были отброшены три гипотезы: «чип залип и нужен сброс
питания», «чип остался в четырёхбитном режиме», «правка оставила пин без
сброса». Ни одна не подтвердилась — при захвате на 50 МГц те же прошивки
читаются чисто.

Правило: **прежде чем объяснять `dev=0` поведением чипа, перезахвати на вдвое
большей частоте**. Признак искажения именно в декоде, а не в железе — мусор в
БУКВАХ постоянной части строки (`cfg`, `cur`, `dev`): их прошивка печатает
константами, и портиться им неоткуда.

Сопутствующее правило по диагностике: снимок состояния делать ЛИПКИМ (писать
только первым срабатыванием сторожа) и печатать рядом текущее состояние.
Перезаписываемый снимок показывает само состояние аварии вместо места затыка.

Проверенное попутно: замена `assign LTDC_REINIT = 1'b0;` на регистр, который
при выключенном параметре нигде не присваивается, даёт ПОБИТОВО ТОТ ЖЕ
битстрим (тот же User Code) — синтезатор такой регистр выбрасывает. То есть
«регистр без сброса» в этом случае мину не создаёт; писать константу всё равно
правильнее, но объяснять ею отказы железа нельзя.

**Диагностика врёт, если статус печатается СНИМКОМ состояния.** В строке `#ST`
поле `st` выводится ШЕСТНАДЦАТЕРИЧНО: `st=10` — это состояние 16 (`LS_ERROR`),
а вовсе не десятое. На этом легко потерять час. Поле `cfg=0` при этом ничего не
доказывает: регистр ошибки инициализирован нулём, и если сверка ни разу не
завершилась, он так нулём и останется. Читать связку целиком: `dev` — вот
единственный признак того, что чип вообще отвечает по SPI.
