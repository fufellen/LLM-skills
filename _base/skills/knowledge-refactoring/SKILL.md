...
- Concept-note scoping (essence vs. implementation): in a note whose subject is concept X, keep the essence/principle of X. Implementation details of the *example* used to illustrate it — another subsystem's registers, peripheral internals, SPI/bus dumps, exact device constants, register-level HDL, waveform screenshots — are tonkosti that belong in that example's own canonical note; in the concept note trim them to a one-line illustration plus a `[[link]]`. The reader of the concept note wants the concept, not a tour of the example's implementation. When trimming, relocate the unique detail into the example's note rather than deleting it (user feedback, 2026-07-07).
