...
| board-specific implementation | платная реализация (прилагательное от «плата» совпадает с «платный» — за деньги) | реализация для конкретной платы; привязанная к плате; зависящая от аппаратного исполнения |
...
Отдельная проверка на неоднозначность, не связанная с английским. Прилагательное,
образованное от технического существительного, негодно, если совпадает с обиходным
словом другого значения: «платная реализация» (от «плата») читается как «за деньги».
Проверка простая: показать слово без окружения и спросить, что оно значит; если ответ
не тот, признак называют прямо оборотом — «реализация для конкретной платы»,
«привязанная к плате», «зависящая от аппаратного исполнения». Правило шире одного
слова: ярлык из одного прилагательного почти всегда проигрывает обороту из двух-трёх
слов, который называет суть без догадок.
