...
8. **Раскрывай сжатые формулировки через наблюдаемый или рассчитанный
   ...
   «фазовый контраст», «порядок близких вариантов может меняться» или
   «направление фазовой перестройки». Не формулируй количественный вывод как
   вопрос, на который модели «дают разные ответы», и не пиши «по-разному
   определяют, у какого материала...». Назови сравниваемую величину и оба
   результата прямо: «в модели A меньшее значение получено для материала X,
   а в модели B — для материала Y». Для общего вывода допустимы точные
   формулировки «знак изменения действительной части эффективного показателя
   преломления» или «материал с наименьшей длиной фазового сдвига».
   ...
   укажи диапазон, отношение или процент и сравниваемую величину; если их
   нет, убери оценочное прилагательное (explicit user correction,
   2026-07-12).
...
12. **Разделяй перегруженные предложения по логическим действиям.** Одно
    ...
    Не строй вводный абзац как цепочку служебных фраз «цель работы»,
    «отдельно оценивается», «также рассматривается», если за ними скрывается
    обычная последовательность расчёта. Сначала назови, что необходимо
    рассчитать, затем — какие величины получают и с чем их сравнивают;
    формулировку цели оставь одним коротким предложением.
...
13. **Разворачивай неоднозначные цепочки родительного падежа.** Фразы вроде
    «два решения модели структуры» или «две моды структуры» могут быть
    грамматически допустимы, но заставляют читателя восстанавливать связи
    между существительными. Сначала назови рассматриваемый объект, затем
    отдельным предложением — что для него рассчитано: «рассмотрена структура
    Si/Au/Si; для неё получены два решения дисперсионного уравнения». Не
    заменяй такую цепочку сложным прилагательным вроде «двухмодовая
    структура», если работа не устанавливает, что структура поддерживает
    ровно две моды: такая замена меняет физический смысл (explicit user
    correction, 2026-07-16).
14. **Проверяй термины-сироты при первом употреблении.** Аннотацию и
    основной текст считай отдельными областями: расшифровка в аннотации не
    заменяет введения сокращения в основном тексте. До первого употребления
    должны быть кратко пояснены специальные термины, сокращения, символы и
    локальные обозначения вроде S1–S3, если их смысл нельзя восстановить из
    самой фразы. Предпочитай короткое физическое пояснение («поле слабо
    удерживается в волноводе») словарному определению; расчётный ярлык
    заменяй описанием действия. Не превращай статью в учебник: общепринятые
...
| pilot calculation / pilot run | пилотный расчёт; пилотный запуск | если этап действительно нужно отличить — предварительный расчёт; если стадия не важна — сразу назвать действие или результат: «при расчёте найдены две моды» |
...
| dielectric-loaded waveguide | диэлектрически нагруженный волновод; волновод с диэлектрической нагрузкой, если «нагрузка» может быть понята как электрическая | назвать геометрию прямо: «волновод с диэлектрическим гребнем над металлической полоской»; после первого пояснения использовать LR-DLSPP |
...
| full-vector calculation / full-wave calculation | полно-векторный / полновекторный / полно-волновой / полноволновой расчёт | не заменять одним новым ярлыком; назвать задачу и метод: при первом упоминании «расчёт собственных мод поперечного сечения волновода с полоской конечной ширины методом конечных элементов», далее по смыслу «расчёт методом конечных элементов», «конечно-элементная модель», «проверка распределения поля», «расчёт потерь» |
| ridge / ridge waveguide | выступ; выступающий волновод (при описании установленной волноводной геометрии) | гребень; гребневый волновод. При первом упоминании пояснить расположение или форму гребня обычной фразой |
| gold strip / gold stripe | микрополосок для элемента шириной 100 нм; микрополосковая линия (если речь лишь о металлическом элементе конечной ширины) | при первом упоминании «полоска золота», далее «золотая полоска» или «металлическая полоска». «Микрополосок» неоднозначен и употребляется для микрометровых полосок в других областях; «микрополосковая линия» — другой тип структуры |
...
| reference root / reference solution (numerical continuation) | опорный корень; опорное решение без пояснения | ранее выбранный корень; указать, из какой задачи он получен и какое новое решение выбирают по близости к нему |
...
| mode-branch tracking | отслеживание модовой ветви | подтверждение того, что при изменении параметра выбирается одна и та же мода |
...
| run/start a numerical search from several initial guesses | поиск корней запускается из нескольких начальных приближений | поиск корней выполняют несколько раз, задавая различные начальные приближения; поиск повторяют для нескольких начальных приближений |
| maximum normalized field overlap | решение с наибольшим нормированным интегральным перекрытием распределений поля | сначала назвать физическое действие: «профиль поля решения наиболее похож на профиль предыдущей моды»; затем при необходимости пояснить критерий: «сходство оценивают нормированным перекрытием; 1 означает совпадение формы профилей» |
| automatic root/mode selection by eigenvalue order | автоматическое правило выбора; простая сортировка корней; номер собственного значения гарантирует выбор моды; физические выводы «для выбранных корней» | в методике явно назвать критерий сопоставления решений; в результатах назвать физическую неоднозначность и формулировать выводы для сопоставленных мод |
...
2. Grep the body for, at minimum:
   ...
   (не исправлять только написание: заменить общий ярлык конкретным названием
   задачи и метода), `лоссов`,
   ...
   `отвечает пределу`, `быстрый инструмент`, `пилотн\w*\s+(расч[её]т|запуск|модел)`, `локальн` (локально
   ...
   `поиск\w*.*запуска|запуска\w*.*поиск` (в описании научного метода назвать
   выполняемое вычислительное действие),
   `(автоматическ\w*\s+правил|прост\w*\s+сортировк\w*\s+корн|номер\w*\s+собственн\w*\s+значен|выбранн\w*\s+корн)`
   (заменить описание порядка выбора корней физическим различием мод и
   указать способ их сопоставления),
   `опорн\w*\s+(кор|решен)` (назвать, из какой задачи взят этот корень или
   решение и для выбора какого следующего решения он используется),
   ...
   `порядок\s+близк\w*\s+вариант` (назвать критерий сравнения и конкретные
   варианты),
   `(порядок|ранжирован\w*)\s+материал` (переписать через вывод о том, у
   какого материала меньше или больше явно названная величина),
   `дают\s+разн\w*\s+ответ\w*\s+на\s+вопрос|по-разному\s+определя\w*,?\s+у\s+какого`
   (убрать вопросительную конструкцию и прямо назвать результат каждой
   модели),
   `(значительн|существенн|заметн)\w*` (проверить наличие числа или явно
   названного критерия),
