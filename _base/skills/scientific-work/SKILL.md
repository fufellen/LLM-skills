...
- After creating or materially editing an Obsidian `.md` note, run `scripts/Test-Note.ps1 <path> -First 24 -Strict`; add `-CheckLinks` when link targets should be verified. Strict validation rejects heading levels `#` through `###` and any heading whose text repeats the filename, so generated notes cannot silently violate the vault's `####`-only topical-heading rule or duplicate Obsidian's visible file title.
